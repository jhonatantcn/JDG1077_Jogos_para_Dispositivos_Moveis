...
{
	public Camera mainCamera; // Câmera do jogo

	float touchesPrevPosDifference, touchesCurPosDifference, zoomModifier;
	// espaço entre toques anteriores, espaço entre toques atuais, modificador do tamanho da câmera
	...
	void Update()
	{
		if (Input.touchCount == 2) // Se existem dois toques na tela
		{
			...
			if (touchesPrevPosDifference > touchesCurPosDifference) // Se o espaço anterior era maior
				mainCamera.orthographicSize += zoomModifier; // Aumenta o tamanho da câmera (ZoomIn)
			if (touchesPrevPosDifference < touchesCurPosDifference) // Se o espaço anterior era menor
				mainCamera.orthographicSize -= zoomModifier; // Diminui o tamanho da câmera (ZoomOut)
		}

		mainCamera.orthographicSize = Mathf.Clamp(mainCamera.orthographicSize, 2f, 10f);
		// Mathf.Clamp dá limtes para o tamanho da câmera, mínimo 2 e máximo 10 (zoom mínimo e zoom máximo)
	}
}